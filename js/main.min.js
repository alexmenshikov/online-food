...
cartSpan.innerText=sumCount;};countingDish();const cleanCart=()=>{const body=document.querySelector("body");const modalCart=document.querySelector(".modal-cart");const delDishesBtn=document.querySelector(".modal-cart__bottom-del");if(delDishesBtn){delDishesBtn.addEventListener('click',()=>{localStorage.removeItem('cart');body.classList.remove("block");modalCart.style.display="none";removeCart();countingDish();});}};const editPositionCart=(arrayCart)=>{const add=document.querySelector(".modal-cart__action-add");const remove=document.querySelector(".modal-cart__action-remove");};const renderCart=()=>{const arrayItems=JSON.parse(localStorage.getItem("cart"));const cartItems=document.querySelector(".modal-cart__items");const cartContent=document.querySelector(".modal-cart__bottom");let sum=0;const item=document.createElement("div");item.classList.add("modal-cart__bottom-item");if(!arrayItems){const div=document.createElement("div");div.classList.add("modal-cart__empty");div.innerText="Ой, тут пока пусто";cartItems.append(div);}else{arrayItems.forEach((item)=>{const div=document.createElement("div");div.classList.add("modal-cart__item");const{id,name,price,count}=item;console.log(`${name} ${price} ${count}`);div.innerHTML=`
