...
cartSpan.innerText=sumCount;};countingDish();const cleanCart=()=>{const body=document.querySelector("body");const modalCart=document.querySelector(".modal-cart");const delDishesBtn=document.querySelector(".modal-cart__bottom-del");if(delDishesBtn){delDishesBtn.addEventListener('click',()=>{localStorage.removeItem('cart');body.classList.remove("block");modalCart.style.display="none";removeCart();countingDish();});}};const editPositionCart=(arrayCart)=>{const cartItem=document.querySelectorAll(".modal-cart__item");const sumHTML=document.querySelector(".modal-cart__bottom-sum");const sumHTMLspan=sumHTML.querySelector("span");let sumPrice=0;cartItem.forEach((item)=>{const add=item.querySelector(".modal-cart__action-add");const remove=item.querySelector(".modal-cart__action-remove");const itemHTML=item.querySelector(".modal-cart__item-title").innerText;add.addEventListener('click',()=>{sumPrice=0;if(arrayCart.some((elem)=>elem.name===itemHTML)){arrayCart.map((elem)=>{if(elem.name===itemHTML){elem.count++;item.querySelector(".modal-cart__action-count").innerHTML=elem.count;}});}
localStorage.setItem("cart",JSON.stringify(arrayCart));arrayCart.forEach((item)=>{sumPrice+=(item.price*item.count);});sumHTMLspan.innerText=`${sumPrice} ₽`;});remove.addEventListener('click',()=>{sumPrice=0;if(arrayCart.some((elem)=>elem.name===itemHTML)){arrayCart.map((elem)=>{if(elem.name===itemHTML){elem.count--;item.querySelector(".modal-cart__action-count").innerHTML=elem.count;}});}
localStorage.setItem("cart",JSON.stringify(arrayCart));arrayCart.forEach((item)=>{sumPrice+=(item.price*item.count);});sumHTMLspan.innerText=`${sumPrice} ₽`;});});};const renderCart=()=>{const arrayItems=JSON.parse(localStorage.getItem("cart"));const cartItems=document.querySelector(".modal-cart__items");const cartContent=document.querySelector(".modal-cart__bottom");let sum=0;const item=document.createElement("div");item.classList.add("modal-cart__bottom-item");if(!arrayItems){const div=document.createElement("div");div.classList.add("modal-cart__empty");div.innerText="Ой, тут пока пусто";cartItems.append(div);}else{arrayItems.forEach((item)=>{const div=document.createElement("div");div.classList.add("modal-cart__item");const{id,name,price,count}=item;div.innerHTML=`
